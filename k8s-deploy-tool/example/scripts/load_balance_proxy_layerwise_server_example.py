...
class ProxyState:
    ...
    def __init__(self, prefiller_instances, decoder_instances, interval=5):
        self.prefillers: List[ServerState] = [
            ServerState(h, p, i) for i, (h, p) in enumerate(prefiller_instances)
        ]
        self.decoders: List[ServerState] = [
            ServerState(h, p, i) for i, (h, p) in enumerate(decoder_instances)
        ]
        ...
        self.prefiller_heap = [(server.score, server.index, server)
                               for server in self.prefillers]
        self.decoder_heap = [(server.score, server.index, server)
    ...
    def _update_prefiller_priority(self, server_idx: int, score: Optional[float] = None):
        """Update the priority of a prefiller server in the heap."""
        server = self.prefillers[server_idx]
        if not server.alive:
            server.score = float("inf")
        else:
            if score is not None:
                server.score = score
            else:
                # Priority based on active_tokens and active_kv_cache
                server.score = server.active_tokens + server.active_kv_cache * 0.3
        # Remove old entry and add new one
        self.prefiller_heap = [(p, i, s) for p, i, s in self.prefiller_heap if i != server_idx]
        heapq.heappush(self.prefiller_heap, (server.score, server_idx, server))  # type: ignore

    def _update_decoder_priority(self, server_idx: int, score: Optional[float] = None):
        ...
        if not server.alive:
            ...
            if score is not None:
                ...
                server.score = server.active_tokens
        # Remove old entry and add new one
        self.decoder_heap = [(p, i, s) for p, i, s in self.decoder_heap if i != server_idx]
        heapq.heappush(self.decoder_heap, (server.score, server_idx, server))  # type: ignore
    ...
    def select_prefiller(self, token_count):  # Changed to synchronous
        ...
        while self.prefiller_heap:
            _, chosen, server = heapq.heappop(self.prefiller_heap)
            ...
            if server.alive:
                ...
                logger.info(f"chosen is {chosen}, {server.host}:{server.port}")

                return chosen
        raise RuntimeError("No healthy prefiller servers available")
    ...
    def select_decoder(self, token_count):  # Changed to synchronous
        ...
        while self.decoder_heap:
            _, chosen, server = heapq.heappop(self.decoder_heap)
            if server.alive:
                # Update the chosen server atomically
                server.active_tokens += token_count
                ...
                return chosen
            logger.info(f"pop decoder alive is False {server.host}:{server.port}")
        raise RuntimeError("No healthy decoder servers available")
    ...
    async def mark_server_failed(self, server: ServerState):
        async with server.server_lock:
            server.failed_count += 1
            if server.failed_count >= 3:
                server.alive = False
                server.score = float("inf")
                logger.info(f"--- server {server.host}:{server.port} marked DOWN ---")

    async def run_health_check(self):
        """ scheduled periodic health check task """
        while True:
            await self.health_check_once()
            await asyncio.sleep(self.health_check_interval)

    async def health_check_once(self):
        all_servers = [(s, "prefiller") for s in self.prefillers] + [(s, "decoder") for s in self.decoders]
        for server, kind in all_servers:
            try:
                r = await server.client.get(f"http://{server.host}:{server.port}/health", timeout=2)
                is_alive = (r.status_code == 200)
            except Exception:
                is_alive = False

            async with server.server_lock:
                if is_alive == server.alive:
                    continue

                if is_alive:
                    server.failed_count = 0
                    server.alive = True
                    logger.info(f"--- server {server.host}:{server.port} RECOVERED ---")

                    if kind == "prefiller":
                        self._update_prefiller_priority(server.index, 0)
                    else:
                        self._update_decoder_priority(server.index, 0)
                    ...
                    if server.failed_count >= 3:
                        server.alive = False
                        server.score = float("inf")
                        logger.info(f"--- server {server.host}:{server.port} MARKED UNHEALTHY ---")

                    if kind == "prefiller":
                        self._update_prefiller_priority(server.index)
                    else:
                        self._update_decoder_priority(server.index)
...
async def lifespan(app: FastAPI):
    ...
    health_task = asyncio.create_task(proxy_state.run_health_check())
    ...
    try:
        ...
        async def generate_stream():
            ...
            try:
                ...
                await proxy_state.mark_server_failed(decoder)
...
async def metaserver(request: Request):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            await proxy_state.mark_server_failed(prefiller)
