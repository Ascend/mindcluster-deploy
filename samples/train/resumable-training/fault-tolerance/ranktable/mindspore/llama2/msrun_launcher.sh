...
# 获取msrun的子进程号，提供给reset_process.py使用
msrun_pid=$!
train_pids=()
while true
do 
   train_pids=()
   if [ -e /proc/$msrun_pid/task/$msrun_pid/children ]
   then
      children=$(cat /proc/$msrun_pid/task/$msrun_pid/children)
      for p in $children
      do
...
export MSRUN_TRAINING_PID_NUM_SELF=$train_pids
export MSRUN_PID_NUM_SELF=$msrun_pid