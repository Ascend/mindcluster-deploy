...
from mindx_elastic.recover_module.recover_manager import init_grpc_client
from mindx_elastic.recover_module import shared_data
...
def start_grpc_client():
    ...
    if ms_role != "MS_SCHED":
        return
    logger.info("start grpc client in reset process")
    init_grpc_client()
    logger.info("start grpc client end")


def signal_mindio_controller_end():
    ms_role = os.getenv("MS_ROLE")
    if ms_role != "MS_SCHED":
        return
    shared_data.shared_data_inst.set_kill_flag(True)
    logger.info("reset process set kill flag")
