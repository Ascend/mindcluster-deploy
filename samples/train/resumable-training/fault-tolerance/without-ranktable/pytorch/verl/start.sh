export HCCL_SOCKET_IFNAME=enp48s3u1u1 # modify according to actual situation
export TP_SOCKET_IFNAME=enp48s3u1u1   # modify according to actual situation
export GLOO_SOCKET_IFNAME=enp48s3u1u1 # modify according to actual situation
...
export NPU_PER_NODE=16  # A3 NPU Number
export NNODES=2         # example is 2 Nodes

export path_log_dir=/home/verl/logs/$MINDX_TASK_ID/trainlog  # modify according to actual situation
export ASCEND_PROCESS_LOG_PATH=/home/verl/logs/$MINDX_TASK_ID/plog # modify according to actual situation
...
if [ "$RANK" = "0" ]; then
  ...
  while [[ $cnt -lt 10 ]]; do
    ...
    if [ "$npu_count_int" -ge "$((NNODES*NPU_PER_NODE))" ]; then
      ...
      ray status
      bash run_dapo_qwen3_30b_a3b_megatron.sh
      break
    fi
